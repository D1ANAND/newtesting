...
async function sendRequest(): Promise<number> {
  const url = "https://estimate-gas.vercel.app/api/estimate-gas";
  const payload = { input_value: "test" };
  ...
  try {
    ...
    if (response.ok) {
      ...
      throw new Error("Failed to fetch random number from API");
    }
  } catch (error) {
...
export const signAndCombineAndSendTx = async () => {
  ...
  try {
    ...
    const randomValueResponse = await sendRequest();
    console.log("API Response:", randomValueResponse);

    const valueInWei = ethers.utils.parseEther(randomValueResponse);
...
// Trigger function via button click
const button = document.getElementById("extractWordButton");
if (button) {
  button.addEventListener("click", () => {
    signAndCombineAndSendTx().then((result) => {
